...
TARGET_COLUMNS = ["payment_type_key", "payment_method", "payment_description", "is_deleted"]
...
def transform():
    silver_df = (
        spark.read.table("silver.payment")
        .select("payment_method")
        .distinct()
    )
    # self.source_row_count = spark.read.table("silver.payment").count()

    source_df = (
        silver_df
        .withColumn(
            "payment_description",
            F.when(F.col("payment_method") == F.lit("E-wallet"), F.lit("Transfer using Electronic Wallet services"))
            .when(F.col("payment_method") == F.lit("Credit Card"), F.lit("Payment through Credit Card (Visa, Mastercard, JCB)"))
            .when(F.col("payment_method") == F.lit("Bank Transfer"), F.lit("Payment using Banking services"))
...
def unknown_member_df():
    unknown_df = spark.createDataFrame(
        [(-1, "Unknown", "Unknown Payment Type", False)],
        [
            "payment_type_key",
            "payment_method",
            "payment_description",
...
def add_surrogate_keys( df, current_max_key):
    ...
    key_window = Window.orderBy("payment_method")
...
def write_to_gold( source_df):
    ...
    existing_payment_type_df = target_df.select("payment_method").distinct()
    new_payment_type_df = source_df.alias("src").join(
        existing_payment_type_df.alias("tgt"),
        on="payment_method",
        how="left_anti",
    )
    ...
    if existing_source_count > 0:
        (
            ...
            .whenMatchedUpdate(
                condition="tgt.payment_method <> 'Unknown'",
                set={
                    "payment_description":"src.payment_description",
    ...
    new_payment_type_count = new_payment_type_df.count()
    if new_payment_type_count > 0:
        current_max_key = target_df.agg(F.max("payment_type_key")).first()[0]
        new_payment_type_df = add_surrogate_keys(new_payment_type_df, current_max_key)
        (
            new_payment_type_df.write.format("delta")
            .mode("append")
            .saveAsTable(TARGET_TABLE)
        )
        processed_rows += new_payment_type_count

    if str(mark_missing_from_source_as_deleted).lower() == "true":
        missing_source_df = target_df.alias("tgt").join(
            source_df.select("payment_method").distinct().alias("src"),
            on="payment_method",
            how="left_anti",
        )
