...
def transform():
    policy = spark.read.table("silver.policy").alias("p")
    # self.source_row_count = policy.count()
    ...
    dim_policy = spark.read.table("gold.DimPolicyInfo").alias("dpi")
    ...
    df = (
        policy
        .join(dim_date, F.col("p.issued_date").cast("date") == F.col("dd.full_date"), "left")
        .join(dim_customer, F.col("p.customer_id") == F.col("dcm.customer_id"), "left")
        .join(dim_provider, F.col("p.provider_code") == F.col("dp.provider_code"), "left")
        .join(dim_quote, F.col("p.quotation_id") == F.col("dqi.quotation_id"), "left")
        .join(dim_policy, F.col("p.policy_id") == F.col("dpi.policy_id"), "left")
        .join(quotation, F.col("p.quotation_id") == F.col("q.quotation_id"), "left")
    ...
    df = df.select(
        ...
        F.col("dd.date_key").cast("int").alias("issue_date_key"),
        ...
        F.col("p.premium_amount").cast("decimal(18,2)").alias("policy_premium_amount")
    )
...
def write_to_gold( df):
    ...
    # Fact Policy don't have any PK, so I use composite key to check
    # May check on these columns to create a better composite key
    business_columns = ["customer_key", "agent_key", "issue_date_key", "provider_key", "policy_info_key", "quotation_info_key"]
